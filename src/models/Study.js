...
const studySchema = new mongoose.Schema(
  {
    ...
    // Admin enters image link manually here
    imageUrl: {
      type: String,
      trim: true,
...
studySchema.virtual("totalLikes").get(function () {
  const commentLikes = this.comments.reduce(
    (sum, comment) => sum + comment.likes.length,
    0
  );
...
studySchema.pre("save", function (next) {
  ...
  if (!this.imageUrl || !this.imageUrl.trim()) {
    this.imageUrl = "/images/bible-study-default.jpg";
  }
