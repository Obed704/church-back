...
// APPLY TO JOIN (requires login on frontend; backend accepts data and stores request)
router.post("/:id/join", async (req, res) => {
  try {
    ...
    const isMember = (dept.members || []).some(
      (m) => String(m).trim().toLowerCase() === String(name).trim().toLowerCase()
    );
    ...
    const alreadyRequested = (dept.joinRequests || []).some((r) => {
      ...
      const sameEmail = email && r.email && String(r.email).toLowerCase() === String(email).toLowerCase();
    ...
    return res.json({ message: "Application submitted. We will contact you soon.", status: "requested" });
  } catch (err) {
