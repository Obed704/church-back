...
router.get("/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error fetching event", error: err.message || err });
  }
});
...
router.post("/", verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    if (!title || !description || !dateValue) {
      return res.status(400).json({ message: "title, description, and dateStart (or date) are required" });
    }
    ...
    res.status(500).json({ message: "Error creating event", error: err.message || err });
  }
});
...
router.put("/:id", verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error updating event", error: err.message || err });
  }
});
...
router.delete("/:id", verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error deleting event", error: err.message || err });
  }
});
...
router.post("/:id/rsvp", verifyToken, async (req, res) => {
  try {
    ...
    const already = ev.attendees.some((a) => String(a.userId) === String(userId));
    ...
    if (already) {
      ev.attendees = ev.attendees.filter((a) => String(a.userId) !== String(userId));
    } else {
      ev.attendees.push({
        userId,
    ...
    res.status(500).json({ message: "Error updating RSVP", error: err.message || err });
  }
});
